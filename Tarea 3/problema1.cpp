...
int MenorDistancia(piedra *rio, int largo)
{
	...
	for (int i = 0; i < largo; i++)
	{
		if (rio[i].tipo == 2)
		{
			if (rio[i + 1].tipo == 2 && rio[i].der >= maximo)
			{ // G --- G
			...
			{ // G --- P
				...
				else if (rio[i + 2].tipo == 1)
				{ //G--P--P
					PosibleSalto2 = rio[i + 2].der + rio[i + 2].izq;
					if (PosibleSalto2 > PosibleSalto)
					{
						maximo = std::max(PosibleSalto, maximo);
					}
					else
					{
						maximo = std::max(std::max(PosibleSalto, PosibleSalto2), maximo);
						i++;
					}
				}
			}
		...
		{ // P
			...
			{ // P --- P
				...
				{ // P --- P --- G
					i++;
					maximo = std::max(std::max(rio[i + 1].izq + rio[i + 1].der, rio[i].izq + rio[i].der), maximo);
				}
				if (rio[i + 2].tipo == 1)
				{ // P --- P --- P
